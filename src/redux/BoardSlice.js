...
const boardSlice = createSlice({
  ...
  reducers: {
    ...
    updateTasklistOrder: (state, action) => {
      ...
      console.log('state.boardLists', JSON.parse(JSON.stringify(state.boardLists)));
      const { draggableId, destination, source } = action.payload;
      console.log('updateTasklistOrder destination, source', destination, source);
      //find the list index of the source of the task
      let boardListIndex = state.boardLists.findIndex((list) => list._id === source.droppableId);
      //Remove the task from sourceIndex and store it
      const [movedTask] = state.boardLists[boardListIndex].taskList.splice(source.Index, 1);
      ...
      if (destination.droppableId !== source.droppableId) {
        ...
        boardListIndex = state.boardLists.findIndex((list) => list._id === destination.droppableId);
      }
      state.boardLists[boardListIndex].taskList.splice(destination.index, 0, movedTask);
    ...
    removeTaskFromBoard: (state, action) => {
      const index = state.boardLists.findIndex((list) => list._id === action.payload.taskList);
      if (index !== -1) {
