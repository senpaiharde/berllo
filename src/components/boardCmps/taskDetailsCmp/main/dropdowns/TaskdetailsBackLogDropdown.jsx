...
const TaskdetailsBackLogDropdown = ({ trigger, onClose, Header }) => {
  ...
  useEffect(() => {
    ...
    setDestination({ droppableId: listId, index: targetPosition });
  }, [position, listId]);
  ...
  const handleMove = () => {
    ...
    dispatch(
      updateTasklistOrderAndSync({ draggableId: task._id, destination, source, copiedTask: null })
    );
    ...
    if (copiedTaskStatus) {
      ...
      dispatch(
        updateTasklistOrderAndSync({
          ...
          destination,
          source,
          copiedTask: transformedCopiedTask,
        })
      );
            ...
            <div className="keepSection">
              {task.checklist.length > 0 && (
                ...
                >
                  ...
                  <label
                    ...
                    style={{ marginTop: '12px' }}>{`checklist (${task.checklist.length})`}</label>
                </div>
              )}
              {task.labels.length && (
                <div
                ...
                >
                  ...
                  <label
                    className="checklistAvi "
                    style={{ marginTop: '12px' }}>{`Labels (${task.labels.length})`}</label>
                </div>
              )}
              {task.members.length > 0 && (
                <div
                  ...
                  <label
                    className="checklistAvi "
                    style={{ marginTop: '12px' }}>{`Members (${task.members.length})`}</label>
                </div>
              )}
              ...
              {task.comments.length > 0 && (
                <div
                  ...
                  <label
                    className="checklistAvi "
                    style={{ marginTop: '12px' }}>{`Comments (${task.comments.length})`}</label>
                </div>
              )}
            </div>
