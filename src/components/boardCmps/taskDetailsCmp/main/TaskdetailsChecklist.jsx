...
const TaskChecklist = () => {
  ...
  const taskMembers = Array.isArray(selectedTask?.checklist) ? selectedTask.checklist : [];
  if (!selectedTask || !Array.isArray(selectedTask.checklist)) return null;

  const handleToggleCheck = (groupTitle, itemId) => {
  ...
  const handleEditText = (groupTitle, itemId, text) => {
    const updated = selectedTask.checklist.map((group) =>
      group.title !== groupTitle
        ? group
        : {
            ...group,
            items: group.items.map((item) => (item.id === itemId ? { ...item, text } : item)),
          }
    );
  ...
  const handleDeleteGroup = (groupId) => {
    console.log('removing to group:', groupId);
    if (!selectedTask) return;

    // Remove the entire group with this _id
    const updatedChecklist = selectedTask.checklist.filter(
      (group) => group._id.toString() !== groupId
    );

    // Dispatch exactly like your working member/label handlers
    dispatch(
      liveUpdateTask({
        method: TaskOps.UPDATE,
        workId: 'tasks',
  ...
  const handleAddItem = (groupId) => {
    ...
    const updated = selectedTask.checklist.map((group) =>
      group._id.toString() === groupId ? { ...group, items: [...group.items, newItem] } : group
    );

    console.log('Adding to group:', groupId, updated);
    dispatch(
      liveUpdateTask({
        method: TaskOps.UPDATE,
        workId: 'tasks',
  ...
  const handleClearGroup = (groupTitle) => {
    const updated = selectedTask.checklist.map((group) =>
      group.title !== groupTitle ? group : { ...group, items: [] }
    );
            ...
            {visible.map((item) => (
              <div
                ...
                <input
                  type="checkbox"
                  checked={item.done}
                  onChange={() => handleToggleCheck(group.title, item.id)}
                ...
                <div className="checklist-item-wrapper-Text">
                  <input
                    ...
                    type="text"
                    value={item.text || ''}
                    onChange={(e) => handleEditText(group.title, item.id, e.target.value)}
                    onFocus={(e) => {
                  ...
                  {hovering === item.id && (
                    <div style={{ position: 'absolute', top: '6px', right: '9px' }}>
                      <DropdownChecklist
                        ...
                        onClick={() => handleDeletecheckList(group._id.toString())}
                      />
                    </div>
                  )}
                </div>
