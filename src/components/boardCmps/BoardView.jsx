...
import { addBoardList, updateBoardListOrder, updateBoardlist } from "../../redux/BoardSlice"
...
export function BoardView() {
  ...
  const onDragEnd = (result) => {
    ...
    console.log(
      "onDragEnd result.destination",
      result.destination,
      "result.source",
      result.source
    )
    if (result.type === "task") {
      updateListOrder(result)
    } else {
      dispatch(
    ...
    // dispatch(updateBoardListOrder({sourceIndex: result.source.index, destinationIndex: result.destination.index}));
  }
  ...
  if (sourceListId === destListId) {
    ...
    const listIndex = board.boardLists.findIndex(list => list._id === sourceListId)
    ...
    const sourceListIndex = board.boardLists.findIndex(list => list._id === sourceListId)
    const destListIndex = board.boardLists.findIndex(list => list._id === destListId)
    if (sourceListIndex === -1 || destListIndex === -1) return

    const sourceTaskList = Array.from(board.boardLists[sourceListIndex].taskList)
  ...
  return (
    <div className="board-view">
      <DragDropContext onDragEnd={onDragEnd}>
        <Droppable droppableId="taskLists" direction="horizontal" type="list">
          {(provided) => (
            <div
